...
GtkWidget *create_rx_panadapter(RECEIVER *rx)
{
  ...
  gtk_widget_set_events(panadapter, gtk_widget_get_events(panadapter) | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_BUTTON1_MOTION_MASK | GDK_SCROLL_MASK | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK);
...
void update_rx_panadapter(RECEIVER *rx, gboolean running)
{
  ...
  else
  {
    ...
    if (rx->panadapter_gradient)
    {
      ...
      S9 = floor((rx->panadapter_high - S9) * (double)(rx->panadapter_height - 20) / (rx->panadapter_high - rx->panadapter_low));
