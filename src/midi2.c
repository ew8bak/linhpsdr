...
void NewMidiEvent(enum MIDIevent event, int channel, int note, int val)
{
    ...
    if ((desc->channel == channel || desc->channel == -1) && (desc->event == event))
    {
      ...
      case MIDI_CTRL:
        ...
        else if (desc->type == MIDI_WHEEL)
        {
          ...
          new = 0;
          new = val - 64;
          if ((val >= desc->vfl1) && (val <= desc->vfl2))
            new = -100;
          if ((val >= desc->fl1) && (val <= desc->fl2))
            new = -10;
          if ((val >= desc->lft1) && (val <= desc->lft2))
            new = -1;
          if ((val >= desc->rgt1) && (val <= desc->rgt2))
            new = 1;
          if ((val >= desc->fr1) && (val <= desc->fr2))
            new = 10;
          if ((val >= desc->vfr1) && (val <= desc->vfr2))
            new = 100;
  ...
  if (!desc)
  {
    ...
    if (event == MIDI_PITCH)
      g_print("%s: Unassigned PitchBend Value=%d\n", __FUNCTION__, val);
    if (event == MIDI_NOTE)
      g_print("%s: Unassigned Key Note=%d Val=%d\n", __FUNCTION__, note, val);
    if (event == MIDI_CTRL)
      g_print("%s: Unassigned Controller Ctl=%d Val=%d\n", __FUNCTION__, note, val);
  }
  else if (midi_debug)
  {
    if (event == MIDI_PITCH)
      g_print("%s: PitchBend Value=%d\n", __FUNCTION__, val);
    if (event == MIDI_NOTE)
      g_print("%s: Key Note=%d Val=%d\n", __FUNCTION__, note, val);
    if (event == MIDI_CTRL)
      g_print("%s: Controller Ctl=%d Val=%d\n", __FUNCTION__, note, val);
  }
}
