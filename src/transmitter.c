...
void transmitter_restore_state(TRANSMITTER *tx)
{
  ...
  value = getProperty(name);
  if (value)
    tx->mic_gain = atof(value);
  sprintf(name, "transmitter[%d].linein_gain", tx->channel);
  value = getProperty(name);
  if (value)
    tx->linein_gain = atoi(value);
  sprintf(name, "transmitter[%d].alc_meter", tx->channel);
  value = getProperty(name);
  if (value)
    tx->alc_meter = atoi(value);
  sprintf(name, "transmitter[%d].drive", tx->channel);
  value = getProperty(name);
  if (value)
    tx->drive = atof(value);
  ...
  value = getProperty(name);
  if (value)
    tx->mic_sample_rate = atoi(value);
  sprintf(name, "transmitter[%d].mic_dsp_rate", tx->channel);
  value = getProperty(name);
  if (value)
    tx->mic_dsp_rate = atoi(value);
  /*
    sprintf(name,"transmitter[%d].iq_output_rate",tx->channel);
    value=getProperty(name);
  ...
  */
  ...
  value = getProperty(name);
  if (value)
    tx->pre_emphasize = atoi(value);
  sprintf(name, "transmitter[%d].enable_equalizer", tx->channel);
  value = getProperty(name);
  if (value)
    tx->enable_equalizer = atoi(value);
  for (i = 0; i < 4; i++)
  {
    sprintf(name, "transmitter[%d].eualizer[%d]", tx->channel, i);
    value = getProperty(name);
    if (value)
      tx->equalizer[i] = atoi(value);
  }
  sprintf(name, "transmitter[%d].leveler", tx->channel);
  value = getProperty(name);
  if (value)
    tx->leveler = atoi(value);
  sprintf(name, "transmitter[%d].fps", tx->channel);
  value = getProperty(name);
  if (value)
    tx->fps = atoi(value);
  ...
  value = getProperty(name);
  if (value)
    tx->eer_amiq = atoi(value);
  sprintf(name, "transmitter[%d].eer_pgain", tx->channel);
  value = getProperty(name);
  if (value)
    tx->eer_pgain = atof(value);
  sprintf(name, "transmitter[%d].eer_mgain", tx->channel);
  value = getProperty(name);
  if (value)
    tx->eer_mgain = atof(value);
  sprintf(name, "transmitter[%d].eer_pdelay", tx->channel);
  value = getProperty(name);
  if (value)
    tx->eer_pdelay = atof(value);
  sprintf(name, "transmitter[%d].eer_mdelay", tx->channel);
  value = getProperty(name);
  if (value)
    tx->eer_mdelay = atof(value);
  sprintf(name, "transmitter[%d].eer_enable_delays", tx->channel);
  value = getProperty(name);
  if (value)
    tx->eer_enable_delays = atoi(value);
  sprintf(name, "transmitter[%d].eer_pwm_min", tx->channel);
  value = getProperty(name);
  if (value)
    tx->eer_pwm_min = atoi(value);
  sprintf(name, "transmitter[%d].eer_pwm_max", tx->channel);
  value = getProperty(name);
  if (value)
    tx->eer_pwm_max = atoi(value);
...
int SendTXpacketQuery(TRANSMITTER *tx)
{
  ...
  case 2:
  {
    ...
    if (radio->sample_rate == 48000)
    {
      if (((tx->packet_counter % 0x7) & 0x01) == 0)
        return 1;
      break;
    }
    ...
    if (radio->sample_rate == 96000)
    {
      if (((tx->packet_counter % 0x7) & 0x03) == 0)
        return 1;
      break;
    }
  ...
  default:
  {
    ...
    if (radio->sample_rate == 48000)
      FrameIndex = tx->packet_counter % 63; // FrameIndex is 0..62
    if (radio->sample_rate == 96000)
      FrameIndex = tx->packet_counter % 126; // FrameIndex is 0..125
    if (radio->sample_rate == 192000)
      FrameIndex = tx->packet_counter % 252; // FrameIndex is 0..251
    if (radio->sample_rate == 384000)
      FrameIndex = tx->packet_counter % 504; // FrameIndex is 0..503

    for (int i = 0; i < 26; i++)
    {
      if (FrameIndex == protocol1_tx_scheduler[selector][i])
        return 1;
    }
    break;
  }
...
{
  if (!isTransmitting(radio) && radio->discovered->device != DEVICE_HERMES_LITE2)
    return;
  if ((isTransmitting(radio)) && (radio->classE))
    return;

  // Work out if we are going to send a tx packet or return
  if (!SendTXpacketQuery(tx))
    return;
